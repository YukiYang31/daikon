...
class ReplaceStatementMap {
  ...
  /**
   * Returns the ReplaceStatement whose name is name. If none can be found, then name is
   * repetitively shortened by eliminating each of its leading prefixes in turn. If once all
   * prefixes are removed, still no matching ReplaceStatements can be found, null is returned. (A
   * prefix is defined as a segment of name which is terminated by a "dot".) For example if this
   * contains entries with names "c", "b.c" and "x.y.z", "get(c)" would return "c", "get(a.b.c)"
   * would return "b.c" and "get(y.z)" would return null.
   *
